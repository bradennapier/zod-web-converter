...
export class FactoryCodeEditor extends React.Component<
  FactoryCodeEditorProps,
  FactoryCodeEditorState
> {
  ...
  render() {
    ...
    return <div id={cssConstants.factoryCodeEditor.id}>{this.getEditor()}</div>;
  }
  ...
  private updateFactoryCodeGenerator() {
    ...
    setTimeout(() => {
      ...
      getFactoryCodeGenerator(this.props.compiler.packageName)
        .then((factoryCodeGenerator) => {
    ...
    if (this.state.factoryCodeGenerator === false)
      return (
        <div className={"errorMessage"}>
          Error loading factory code. Please refresh the page to try again.
        </div>
      );
  ...
  private getText() {
    if (
      this.state.factoryCodeGenerator == null ||
      this.state.factoryCodeGenerator === false
    )
      return "";
    console.log("Selected Node: ", this.props.compiler.selectedNode);
    return this.state.factoryCodeGenerator(
      this.props.compiler.api,
      this.props.compiler.selectedNode
    );
  }
}
